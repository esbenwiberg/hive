...
function heuristicScores(task: TaskRow): ScorerResult {
  ...
  if (isTrivial) {
    return {
      ...
      costEstimate: {
        totalUsd: 0.01,
        breakdown: { enrichment: 0.005, execution: 0.003, review: 0.002 },
        reasoning: "Trivial task with minimal token usage",
      },
  ...
  if (isSmall) {
    return {
      ...
      costEstimate: {
        totalUsd: 0.05,
        breakdown: { enrichment: 0.02, execution: 0.02, review: 0.01 },
        reasoning: "Small task with modest token usage and no milestones",
      },
      recommendation: "approve",
