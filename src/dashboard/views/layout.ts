...
function sidebar(user: SessionUser): string {
  ...
  const links = navLinks
    .map(
      (l) => `
      <a href="${l.href}"
         class="flex items-center gap-3 rounded-lg px-3 py-2 text-sm text-slate-400 transition-colors hover:bg-slate-800 hover:text-slate-50">
        ${l.icon}
        ${l.label}
      </a>`,
    )
  ...
  return `
  <aside class="fixed inset-y-0 left-0 z-30 flex w-64 flex-col border-r border-slate-800 bg-slate-900">
    <!-- Brand -->
    <div class="flex h-16 items-center gap-3 border-b border-slate-800 px-6">
      <div class="flex h-8 w-8 items-center justify-center rounded-lg bg-amber-400 text-slate-900">
        <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
          <path stroke-linecap="round" stroke-linejoin="round" d="M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" />
          <path stroke-linecap="round" stroke-linejoin="round" d="M9 9.563C9 9.252 9.252 9 9.563 9h4.874c.311 0 .563.252.563.563v4.874c0 .311-.252.563-.563.563H9.564A.562.562 0 0 1 9 14.437V9.564Z" />
        </svg>
      </div>
      <span class="text-lg font-bold tracking-tight text-slate-50">The Hive</span>
    </div>
...
<body class="bg-slate-900 text-slate-50 font-sans">
  ...
  <div class="ml-64 min-h-screen">
    <!-- Topbar -->
    <header class="sticky top-0 z-20 flex h-16 items-center border-b border-slate-800 bg-slate-900/80 px-8 backdrop-blur">
